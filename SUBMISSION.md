...
- **24 tools** registered through `document.modelContext` / `navigator.modelContext`
  ...
  `reveal_area`, `set_scene`), the reach-enforcing `attack`, game flow (`roll_dice`, `narrate`, `start_combat`, `award_loot`)
...
- **Dynamic registration, the way the spec prescribes:** combat tools
  ...
  20, then 23 during combat, then 20 again — and 21 the moment a hero drops.
- **No flag, no setup, for anyone:** the page vendors the MIT
  `@mcp-b/webmcp-polyfill`, so `document.modelContext` and all 24 tools are real
