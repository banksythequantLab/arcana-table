...
```bash
npm install
npm test          # every suite, in order — 213 assertions
```
...
## The suites — `npm test` runs all eight
...
| `reach.mjs` | 25 | Position is a rule: a melee attack out of range is refused with the cell to move to, spells work at distance and are bounded too, Mira's fireball bursts on a cluster and never on the party, and attacking or starting a fight lifts the fog on what you are fighting. |
