...
```bash
npm install
npm test          # every suite, in order — 486 assertions
```
...
| `fixes.mjs` | 41 | Bugs a player actually hit, each with the check that would have caught it — the warm-up spanning the body rather than circling the neck, holds not being validated against the reps list, the party moving as one, and a walk lighting its whole corridor. |
...
| `walk.mjs` | 17 | Clicking the map walks the party there — the first thing every player tries, and until now the only way to move anything was to drag a token. Also the board's only PIXEL check: a scope bug once made every monster vanish from the canvas while all 196 assertions still passed, because nothing asserted the board had drawn anything. |
