...
```bash
npm install
npm test          # every suite, in order — 435 assertions
```
...
| `milestone.mjs` | 59 | Clearing a beat pays, visibly and more each time — loot, a banked boon, a short rest to full — with a banner that names all three. Also that no timer is a trap: a running hold, a 25-minute Oath and a ten-minute warm-up can each be left at any point, and the Cinder Wight has its own art at twice the size. |
